...
const routeList = async () => {
  ...
  exec(
    tsNodePath +
      " -e 'import Server from \"./src/core/Server\"; console.table(Server.initApplication().initServer().application.routes.map(r => { const {schema, ...rest} = r; return rest; }))'",
    (e, o, er) => {
      e && console.log(e);
      er && console.log(er);
      console.log(o);
...
const build = () => {
  ...
  exec(rimrafPath + " ./dist && " + tscPath, async (e, o, err) => {
    e && console.error(e);
    err && console.error(err);
    console.log(o);
    await configureDist();
